...
import { TDepartmentPayload, departmentService } from '../_service/department-service';
...
export function useDepartment() {
  ...
  const { mutate: createDepartment } = useMutation({
    mutationFn: (payload: TDepartmentPayload) => departmentService.create(payload),
    onSuccess: refetchData,
    onError: (error) => console.error('Erro ao criar departamento:', error),
  });

  const { mutate: updateDepartment } = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: TDepartmentPayload }) =>
